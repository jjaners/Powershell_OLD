...
{
    foreach ($AppG in $AppGroup.Split(','))
    {
        #write-host "$AppG"
        #$AG = $AppG | select -ExpandProperty name
        
        $GroupExists = Get-ADGroup -Identity $AppG -Properties name | select -ExpandProperty name
        
            if ($GroupExists -eq $Null)
            {
                Write-host "$GroupExists is Not AD" -ForegroundColor Red
                $Result01 += 1
            }
            #>
            #else
            #{
            #    Write-host "$GroupExists Exists in AD" -ForegroundColor Green
            #}
    }
    
    #$AppG = $AppGroup.Split(',') 
    #$AppG | ForEach-Object {"$AppG"} | FL
}
    #$Singelgroups = @({foreach ($app in $AppGroup.Split(','))})
    
    #foreach ($SingelGroup in $SingelGroups)
    #{
        
     #if ($SingelGroup -eq $true)
    #{
        #write-host "$SingelGroup exist" -ForegroundColor green
...
<#
    ...
    {
        write-host "$SingelGroup doesn't exist" -ForegroundColor Red
    }
  }
}
#>
