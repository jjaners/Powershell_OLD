...
$Results | export-csv -Path D:\Powershell\Logs\Move-Shared-Mailboxes\$Batchname.csv -Delimiter ";" -Encoding UTF8 -NoTypeInformation
...
foreach ($user in $users)
{
$Log += get-MoveRequest -Identity $user -DomainController $DCTieto | select DistinguishedName,Status
}
$Log | Export-Csv D:\Logs\Move-Shared-Mailboxes\Shared_Mailbox_Resource_$dateshort.csv -NoTypeInformation -Encoding UTF8
